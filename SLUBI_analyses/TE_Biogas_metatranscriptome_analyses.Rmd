...
enriched.GO.wall %>% 
  filter(ontology=="BP") %>%
  head(50) %>%
  ggplot(aes(x=numDEInCat/numInCat*100, y=term)) + # plot with ggplot!
...
## Summary table with genes and functional annotation

```{r}
gene.go_ko <- read_delim("SLUBI_analyses/gene_enrichment/all.proteins.emapper.out.emapper.annotations", delim = "\t", col_names = FALSE, comment = "#") %>%
...
gene.go_ko %>% 
  ...
  saveWidgetFix(file="SLUBI_analyses/export/gene_annotation.html", title="Gene annotations")
```

...
metadata.anno.row <- select(metadata_bins.2, c(Class, Contamination, Completeness, Genus))
rownames(metadata.anno.row) <- metadata_bins.2$Bin
metadata.anno.row <- metadata.anno.row %>% data.table::setorder(cols=Class)

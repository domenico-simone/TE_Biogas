...
Analysis is performed with the Wallenius method (default for goseq). Following, three plots of top 50 over-represented GO terms (by adjusted p-value) for each GO ontology (BP = Biological Process, MF = Molecular Function, CC = Cellular Component).
...
```{r}
# read GO terms
genes_KO <- read.table("SLUBI_analyses/gene_enrichment/all.proteins.emapper.ko.pathways", header=FALSE, sep="\t")
genes_KO <- data.frame(base::lapply(genes_KO, function(x) {
    gsub("ko:", "", x)
}))

# run goseq by testing KEGG
KO.wall <- goseq(pwf, gene2cat = genes_KO, test.cats=c("KEGG"))
